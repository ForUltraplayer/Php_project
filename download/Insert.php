...
if ($mode == "modify") {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($del_ok[$i] == "y") {
            ...
            try {
                ...
                $sql = "update project.download set $field_org_name = ?, $field_real_name = ? where num=?";
                $stmh = $pdo->prepare($sql);
    ...
    try {
        $pdo->beginTransaction();
        $sql = "update project.download set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql);
    ...
    try {
        $pdo->beginTransaction();
        $sql = "insert into project.download(id, name, nick, subject, content, regist_day, hit, is_html, ";
...
header("Location:http://localhost/project/download/list.php");
