...
$total_sql = "SELECT COUNT(*) FROM project.download";
...
if ($mode == "search") {
    if (!$search) {
        ...
        $sql = "SELECT * FROM project.download WHERE $find LIKE '%$search%' ORDER BY num DESC LIMIT $page_start, $list";
        $count_sql = "SELECT COUNT(*) FROM project.download WHERE $find LIKE '%$search%'";
        $count_result = $pdo->query($count_sql);
        $count = $count_result->fetchColumn();
    }
} else {
    $sql = "SELECT * FROM project.download ORDER BY num DESC LIMIT $page_start, $list";
}
