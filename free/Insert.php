...
if ($mode == "modify") {
    ...
    try {
        $sql = "select * from project.free where num=?";
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($del_ok[$i] == "y") {
            ...
            try {
                ...
                $sql = "update project.free set $field_org_name = ?, $field_real_name = ? where num=?";
                $stmh = $pdo->prepare($sql);
    ...
    try {
        $pdo->beginTransaction();
        $sql = "update project.free set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql);
    ...
    try {
        $pdo->beginTransaction();
        $sql = "insert into project.free(id, name, nick, subject, content, regist_day, hit, is_html, ";
...
header("Location:http://localhost/project/free/list.php");
