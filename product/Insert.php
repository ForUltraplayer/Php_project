...
if ($mode == "modify") {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($del_ok[$i] == "y") {
            ...
            try {
                ...
                $sql = "update project.product set $field_org_name = ?, $field_real_name = ? where num=?";
                $stmh = $pdo->prepare($sql);
    ...
    try {
        $pdo->beginTransaction();
        $sql = "update project.product set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql);
    ...
    try {
        $pdo->beginTransaction();
        $sql = "insert into project.product(id, name, nick, subject, content, regist_day, hit, is_html, ";
