...
$total_sql = "SELECT COUNT(*) FROM project.qna";
...
if ($mode == "search") {
    if (!$search) {
        ...
        $sql = "SELECT * FROM project.qna WHERE $find LIKE '%$search%' ORDER BY num DESC LIMIT $page_start, $list";
        $count_sql = "SELECT COUNT(*) FROM project.qna WHERE $find LIKE '%$search%'";
        $count_result = $pdo->query($count_sql);
        $count = $count_result->fetchColumn();
    }
} else {
    $sql = "SELECT * FROM project.qna ORDER BY num DESC LIMIT $page_start, $list";
}
        ...
        <div id="col2">
            ...
            <div id="list_content">
                ...
                while ($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $sql = "select * from project.qna_ripple where parent=$item_num";
                    $stmh1 = $pdo->query($sql);
